...
const Join = () => {
  ...
  return (
    <div className="min-h-[80vh] flex flex-col justify-center py-16 px-4">
      <SEO title="Join the Movement | Eunoia Society" description="Become a member, volunteer, or partner." />
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center mb-16">
          ...
          <p className="text-xl font-bold opacity-70 max-w-2xl mx-auto">
            Choose how you want to help us redesign how Bangladesh works.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {options.map((opt, i) => {
            const isMember = opt.title === 'Member';
            const memberHover = {
              x: -4, 
              y: -4, 
            ...
            return (
              <BrutalistCard key={i} className={`flex flex-col h-full items-center text-center p-12 ${opt.color}`}>
                ...
                <Link to={opt.path} className="w-full">
                  <BrutalistButton 
                    variant={isMember ? 'accent' : (opt.title === 'Partner' ? 'primary' : 'outline')} 
                    fullWidth 
                    className="!shadow-none hover:translate-x-1 hover:translate-y-1"
                    {...(isMember ? { whileHover: memberHover } : {})}
                  >
                    Start Application
                  </BrutalistButton>
                </Link>
